...
function cusJsLoad(doc, formEvent, commandControlInfo) {
    ...
    module = $("#ctrl_1006020", doc);
    module.val(pvId); /*模块id*/

    $("#tblForm tr td", doc).eq(0).css("width", "300px");

    /*外键设置为 0 ctrl_1006070*/
    var foreign = $("#ctrl_1006070", doc);
    if (foreign.val() == "") foreign.val(0); /*视图id*/

    /*视图类型的下拉列表框*/
    var viewKind = $("#ctrl_1006030", doc);
    viewKind.change(function () {
        showTr(this.value, doc);
    });

    showTr(viewKind.val(), doc);
    
    /*表ID的下拉列表框*/
    $("#ctrl_1006050", doc).change(function () {

        showTable(this.value, doc);
