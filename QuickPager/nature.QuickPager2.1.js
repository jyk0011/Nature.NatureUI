...
Nature.Page.QuickPagerExtend = {
    CreateNavi: function (pageInfo) {
        ...
        if (s1 >= 2) {
            temp = n1.replace("{text}", 1);navi.append(temp);
        }
        if (s1 >= 3) {
            temp = n1.replace("{text}", "…");navi.append(temp);
        }
        ...
        if (s2 <= pageInfo.pageCount - 1) {
            temp = n1.replace("{text}", pageInfo.pageCount);navi.append(temp);
        }
    ...
    KeyPager: function () {
        //判断焦点。焦点在文本框这类的控件的时候，不触发事件
        switch (event.srcElement.type) {
            case "text":case "hidden":case "password":case "file":case "textarea":case "checkbox":case "radio":
            case "select-one":case "select-multiple":
                return;
        }
        var keyCode = window.event.keyCode;
        var txt = document.getElementById("Txt_GO");
        var go = document.getElementById("P_GO");
        //alert(keyCode);
        //33:pageup；34pagedown；37<- ；39->
        switch (keyCode) {
            case 37: case 33:
                document.getElementById("S_Prev").click();break;
            case 39: case 34:
                document.getElementById("S_Next").click();break;
        }
        if (txt) {
            if (go) {
                switch (keyCode) {
                    case 49: case 97: txt.value = "1"; go.click(); break;
                    case 50: case 98: txt.value = "2"; go.click(); break;
                    case 51: case 99: txt.value = "3"; go.click(); break;
                    case 52: case 100: txt.value = "4"; go.click(); break;
                    case 53: case 101: txt.value = "5"; go.click(); break;
                    case 54: case 102: txt.value = "6"; go.click(); break;
                    case 55: case 103: txt.value = "7"; go.click(); break;
                    case 56: case 104: txt.value = "8"; go.click(); break;
                    case 57: case 105: txt.value = "9"; go.click(); break;
                    case 48: case 96: txt.value = "10"; go.click(); break;
                }
            }
        }
    }
...
Nature.Page.QuickPager = function () {
    ...
    this.Start = function (callback) {
        ...
        document.body.onkeydown = info.KeyPager;
    ...
    var setDefaultInfo = function (a) {
        ...
        if (a.KeyPager == undefined) a.KeyPager = Nature.Page.QuickPagerExtend.KeyPager;
