...
//主函数
Nature.Controls.Form.prototype.checkForm = function() {

    var self = this;
    
    var oForm;
    if (window.navigator.appName.toLowerCase().indexOf("microsoft") > -1) {
        oForm = self.formEvent.window.document.forms["dataForm"];
    } else {
        oForm = self.formEvent.window.document.forms["dataForm"];
    }
    ...
    function getValue(el) {
        ...
        function getValueChoose(el) {
            ...
            var tmpels = self.formEvent.window.document.getElementsByName(el.name);
...
//取得select的选中数,用"0"来表示选中的个数,我们写正则的时候就可以通过0{1,}来表示选中个数
    ...
    function GoBack(el) {
        ...
        case "checkbox":
        case "radio":
            var els = self.formEvent.window.document.getElementsByName(el.name);
            els[0].focus();
            break;
        case "select-one":
        case "select-multiple":
